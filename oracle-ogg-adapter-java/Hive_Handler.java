...
public class SampleHandlerHive  extends AbstractHandler {
  final private static Logger logger = LoggerFactory.getLogger(SampleHandlerHive.class);
...
/** count total number of transactions */
  ...
  public SampleHandlerHive() {
    ...
    logger.info("Created handler: default mode=" + getMode() + ", default hive filename=" + hdfsFileName);
  }

public void init(DsConfiguration conf, DsMetaData metaData) {
    logger.info("Initializing handler: mode=" + getMode()+ ", Hive filename="+ hdfsFileName);
    ...
    try {
      ...
      throw new ConfigException("Can't initialize file, " + regularFileName, ioe);
    }
   ...
   // Initialize HDFS
    try {
      ...
      throw new ConfigException("Can't initialize HDFS file, " + hdfsFileName, ioe);
    }
...
public void setRecordDelimiter(String delim) {
 ...
 public Status transactionBegin(DsEvent e, DsTransaction tx) {
    ...
    String eventFormat ;
    eventFormat = String.format("Received begin tx event, numTx=%d : position=%s%n", numTxs , tx.getTranID());
...
public Status operationAdded(DsEvent e, DsTransaction transaction, DsOperation operation) {
    ...
    out.println("  Received operation: table='" + op.getTableName() + "'"
      + ", pos=" + op.getPosition()
      + " (total_ops= "+ tx.getTotalOps()+ ", buffered="+ tx.getSize()+ ")"
      + ", ts=" + op.getTimestamp());
  ...
  public Status transactionCommit(DsEvent e, DsTransaction transaction) {
    ...
    out.println("  Received commit event, tx #" + numTxs + ": "
      + ", pos=" + tx.getTranID()
      + " (total_ops= "+ tx.getTotalOps()+ ", buffered="+ tx.getSize()+ ")"
      + ", ts=" + tx.getTimestamp() + ")");
    out.flush();
  ...
  private void processOp(Tx currentTx, Op op) {
    if(logger.isDebugEnabled()) logger.debug("processOp: tx=" + currentTx.getPosition() + ", op=" + op.getPosition());
    numOps++;

    try {
    out.println("  ===> Processing operation: "
      + ", table='" + op.getTableName() + "'"
      + ", pos=" + op.getPosition()
      + " (total_ops= "+ currentTx.getTotalOps()+ ", buffered="+ currentTx.getSize()+ ")"
      + ", ts=" + op.getTimestamp());
  ...
  public Status metaDataChanged(DsEvent e, DsMetaData meta) {
    ...
    out.println("Received metadata event: " + e + "; current tables: " + meta.getTableNames().size());
    return Status.OK;
  }
 ...
 public String reportStatus() {
    ...
    String s = "Status report: file=" +  hdfsFileName
        + ", mode=" + getMode()
        + ", transactions=" + numTxs
        + ", operations=" + numOps;
    return s;
